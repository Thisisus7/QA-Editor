...
const EditorPage = () => {
    ...
    const handleAnswerChange = (e, index) => {
        ...
        if (e.target.value.includes("&&")) {
            newJsonl[currentIndex].answer = e.target.value.split("&&").map(Number);
        } else {
    ...
    const handleNext = () => {  // next question
        const newIndex = (currentIndex + 1) % files.jsonl.length;
        ...
        <div className='editor-content'>
            ...
            <div className='editor-controls'>
                ...
                <>
                    ...
                    <TypeComponent key={`type-${currentIndex}`} type={files.jsonl[currentIndex].type} handleTypeChange={handleTypeChange} />
                    <TextComponent key={`text-${currentIndex}`} text={files.jsonl[currentIndex].text} handleTextChange={handleTextChange} />
                </>
                }
            </div>
